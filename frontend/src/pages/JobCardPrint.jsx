...
const COMPANY_INFO = {
  name: "SENEVI AUTO CARE",
  address: "Mathawa, Wewagama",
  phone: "Tel - 0765336448",
};
      ...
      {jobCard ? (
        <article className="job-card-print-sheet">
          ...
          <header className="job-card-print-header">
            ...
            <p>{COMPANY_INFO.address}&nbsp;&nbsp;&nbsp;{COMPANY_INFO.phone}</p>
          </header>

          {/* ── Row 1: Job Card | Customer | Vehicle ── */}
          <div className="job-card-print-row3">
            <section className="job-card-print-section">
              <h3>Job Card</h3>
              <div className="job-card-print-kv">
                <div><span>Job No</span><strong>{jobNo}</strong></div>
                <div><span>Date / Time</span><strong>{createdAt}</strong></div>
                <div><span>Pay Method</span><strong>{jobCard?.paymentStatus || "-"}</strong></div>
              </div>
            </section>
            ...
            <section className="job-card-print-section">
              ...
              <div className="job-card-print-kv">
                <div><span>Vehicle No</span><strong>{vehicle?.vehicleNumber || "-"}</strong></div>
          ...
          {/* ── Row 2: Appointment | Service Types | Customer Requests ── */}
          <div className="job-card-print-row3">
            <section className="job-card-print-section">
              <h3>Appointment / Delivery</h3>
              <div className="job-card-print-kv">
                <div><span>Appointment</span><strong>____________________</strong></div>
                <div><span>Expected Delivery</span><strong>____________________</strong></div>
              </div>
            </section>
          ...
          {/* ── Full width: Task table + manual write space ── */}
          <section className="job-card-print-section">
            <h3>Service Engineer Instructions / Tasks</h3>
            <table className="job-card-print-table job-card-print-table--tasks">
              <thead>
                <tr>
                ...
                {taskRows.length === 0 ? (
                  ...
                  taskRows.map((task) => (
                    <tr key={task.id}>
                      ...
                      <td className="job-card-print-table__num">
                        {task.laborHours === "" ? "-" : Number(task.laborHours).toFixed(2)}
                      </td>
                      <td className="job-card-print-table__num">
                        {task.laborCharge === "" ? "-" : formatMoney(Number(task.laborCharge))}
                      </td>
                    </tr>
                  ))
                )}
            ...
            {/* blank area for manual task notes during service */}
            <div className="job-card-print-write-space" />
          </section>

          {/* ── Bottom: Materials | Signatures ── */}
          <div className="job-card-print-row2">
            <section className="job-card-print-section">
              <h3>Materials</h3>
              {partsUsed.length === 0 ? (
            ...
            <section className="job-card-print-section job-card-print-signatures">
